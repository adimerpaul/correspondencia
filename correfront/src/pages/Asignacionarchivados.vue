...
<!--                  <div class="col-sm-2 col-12 q-pa-xs"><q-input dense label="Cod externo" v-model="dato.codexterno" outlined /></div>-->
      ...
      <div class="col-12">
        <q-table
          dense
          title="Correspondencia Archivada"
          :rows="mails"
          :columns="columns"
          :filter="filter"
...
this.$q.loading.hide()
    ...
    misdatos(page = 0,filter,rowsPerPage=10){
      //this.$q.loading.show()
      this.loading=true
      this.$axios.get(process.env.API+'/micorre',{params:{page: page,filter:filter,rowsPerPage: rowsPerPage, estado:'ARCHIVADO' }}).then(res=>{
